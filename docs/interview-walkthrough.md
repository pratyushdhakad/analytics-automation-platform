...
I built a revenue forecasting control tower that separates data trust, forecast accuracy, scenario sensitivity, and executive communication. It creates three years of deterministic synthetic sales, marketing, affiliate, and shipment history; validates six source contracts; compares seasonal and driver-aware models across six rolling origins; keeps a different champion for booked and shipped revenue; and turns seven operating plans into an interactive 13-week dashboard. The entire project is standard-library Python, tested, reproducible, and deployed through CI and GitHub Pages.
...
### 6. Close on engineering quality
...
I would replace CSV extracts with partitioned warehouse models, snapshot original plans at each forecast origin, use an orchestrator with retries and observability, add model-registry approval states, calibrate intervals on held-out residuals, and publish the dashboard behind governed access. The existing contracts and stage boundaries are designed to make those replacements incremental.
...
3. Open `config/forecasting.json` and `config/scenarios.json` to show explicit contracts.
...
6. Open the live dashboard and switch between net and shipped revenue.
7. Finish with `.github/workflows/ci.yml` to show reproducibility is enforced.
